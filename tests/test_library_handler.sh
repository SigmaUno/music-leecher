...
result=$(printf '%s\n' '{"request_id":"play-42","action":"resolve","song":{"title":"teardrop","artist":"massive attack"}}' | ./library-handler --library library.example.json)
...
missing=$(printf '%s\n' '{"action":"resolve","song":{"title":"not in library"}}' | ./library-handler --library library.example.json)
